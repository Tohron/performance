...
namespace Startup.Tests
{
    public class StartupTests
    {
        ...
        public void TestProcessTimeParserLinux()
        {
            ...
            IEnumerable<Counter> counters = parser.Parse(ctfFile, "dotnet", pids, "\"dotnet\" build", logger);
            int count = 0;
        ...
        public void TestProcessTimeParserWindows()
        {
            ...
            IEnumerable<Counter> counters = parser.Parse(etlFile, "dotnet", pids, "\"dotnet\" build", logger);
            int count = 0;
            foreach (var counter in counters)
            {
                Assert.True(counter.Results.Count == pids.Count, $"Counter {counter.Name} is expected to have {pids.Count} results.");
        ...
        public void TestTimeToMainParserLinux()
        {
            ...
            IEnumerable<Counter> counters = parser.Parse(ctfFile, "emptycsconsoletemplate", pids, "\"pub\\emptycsconsoletemplate.exe\"", logger);
            int count = 0;
            foreach (var counter in counters)
            {
                Assert.True(counter.Results.Count == pids.Count, $"Counter {counter.Name} is expected to have {pids.Count} results.");
        ...
        public void TestTimeToMainParserWindows()
        {
            ...
            IEnumerable<Counter> counters = parser.Parse(etlFile, "emptycsconsoletemplate", pids, "\"pub\\emptycsconsoletemplate.exe\"", logger);
            int count = 0;
            foreach (var counter in counters)
            {
                Assert.True(counter.Results.Count == pids.Count, $"Counter {counter.Name} is expected to have {pids.Count} results.");
